...
const RecyclingGame = ({ onClose }: RecyclingGameProps) => {
  ...
  const updateGame = useCallback(
    ...
    [basketX, gameOver, isPlaying, score],
  ); // lives is handled via state setter
  ...
  return (
    <div
      className={`w-full max-w-2xl mx-auto space-y-4 transition-all duration-100 ${shake ? "translate-x-[-5px] rotate-[-1deg]" : ""}`}
    >
      ...
      >
        <div className="absolute top-10 left-10 text-white/40 animate-pulse">
          <Cloud className="h-16 w-16 fill-white" />
        </div>
        <div className="absolute top-20 right-20 text-white/30 animate-pulse delay-700">
          <Cloud className="h-12 w-12 fill-white" />
        </div>
            ...
            <div className="bg-white p-8 rounded-[2rem] shadow-2xl space-y-6 max-w-sm mx-auto">
              ...
              <div>
                ...
                <p className="text-slate-500 mt-2 font-medium">
                  Catch the <span className="text-green-600">Recyclables</span>.
                  Avoid the <span className="text-red-500">Trash</span>.
                </p>
              </div>
              <Button
                size="lg"
                onClick={startGame}
                className="w-full rounded-xl text-lg h-14 bg-green-600 hover:bg-green-700 shadow-green-200 shadow-xl hover:scale-105 transition-all"
              >
        ...
        {gameOver && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/20 backdrop-blur-sm z-30 p-6 animate-in zoom-in-95">
            <div className="bg-white p-8 rounded-[2rem] shadow-2xl text-center border-b-8 border-gray-200 w-full max-w-sm">
              ...
              <p className="text-slate-400 font-bold uppercase tracking-wider text-xs mb-6">
                Items Recycled
              </p>
        ...
        {items.map((item) => (
          ...
          >
            <div
              className={`h-14 w-14 rounded-full flex items-center justify-center shadow-lg border-4 ${item.type === "good" ? "bg-white border-green-400 text-green-600" : "bg-white border-red-400 text-red-500"}`}
            >
        ...
        {floaters.map((f) => (
          <div
            key={f.id}
            className={`absolute font-black text-2xl animate-out fade-out slide-out-to-top-10 duration-700 ${f.color}`}
        ...
        <div
          className="absolute bottom-5 z-20 transition-transform duration-75 ease-out will-change-transform"
      ...
      <p className="text-center text-muted-foreground text-sm font-medium">
        Use your mouse or finger to move the bin!
      </p>
    </div>
  );
};
