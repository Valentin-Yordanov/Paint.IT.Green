        ...
        const nextObjects = prevObjects
          .map((obj) => {
            ...
            if (newObj.type === "fire") {
              ...
              if (newObj.scale < 0.3)
                newObj.scale = Math.min(0.3, newObj.scale + dt);
            }
            return newObj;
          })
          .filter((obj) => {
    ...
    <div className="w-full max-w-2xl mx-auto space-y-4 select-none touch-none">
      <div className="flex items-center justify-between bg-card/80 backdrop-blur-sm rounded-2xl p-3 border border-border shadow-sm">
        <div className="flex items-center gap-4">
          <div
            className={`p-2 rounded-xl ${forestHealth < 30 ? "bg-red-100 animate-pulse" : "bg-green-100"}`}
          >
            <TreePine
              className={`h-6 w-6 ${forestHealth < 30 ? "text-red-600" : "text-green-600"}`}
            />
          </div>
      ...
      >
        ...
        <div className="absolute bottom-16 left-[-20px] text-emerald-800/20 blur-[1px]">
          <TreePine className="h-32 w-32" />
        </div>
        <div className="absolute bottom-24 right-[-10px] text-emerald-800/10 blur-[2px]">
          <TreePine className="h-48 w-48" />
        </div>
        <div className="absolute top-10 right-10 text-white/30 animate-pulse duration-[3000ms]">
            ...
            <div className="bg-white p-8 rounded-[2.5rem] shadow-2xl space-y-6 max-w-sm mx-auto border border-white/50">
              ...
              <div className="space-y-2">
                ...
                <p className="text-slate-500 font-medium leading-relaxed">
                  Extinguish{" "}
                  <span className="text-orange-500 font-bold">Fires</span>.
                  Water{" "}
                  <span className="text-green-600 font-bold">Sprouts</span>.
                </p>
              </div>
              <Button
                size="lg"
                onClick={startGame}
                className="w-full rounded-2xl text-lg h-16 bg-sky-500 hover:bg-sky-400 shadow-lg hover:shadow-sky-200/50 hover:scale-[1.02] active:scale-[0.98] transition-all duration-300"
              >
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40 backdrop-blur-md z-40 p-6 animate-in zoom-in-95 duration-300">
            <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl text-center border-4 border-white w-full max-w-sm relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <div className="text-6xl font-black text-transparent bg-clip-text bg-gradient-to-br from-sky-400 to-blue-600 mb-2 tracking-tighter">
                  {score}
                </div>
              ...
              <div className="grid gap-3 relative z-10">
                <Button
                  onClick={startGame}
                  size="lg"
                  className="rounded-xl h-14 text-lg font-bold bg-slate-900 hover:bg-slate-800 text-white shadow-xl"
                >
        ...
        {objects.map((obj) => (
          ...
          >
            <div
              className={`absolute -top-8 left-1/2 -translate-x-1/2 transition-opacity duration-200 ${obj.isWatered ? "opacity-100" : "opacity-0"}`}
            >
            ...
            {obj.type === "fire" ? (
              <div className="relative group">
                <Flame
                  className={`h-14 w-14 text-orange-500 fill-orange-500 drop-shadow-[0_0_15px_rgba(255,165,0,0.6)] ${obj.isWatered ? "animate-pulse scale-90" : "animate-[wiggle_1s_ease-in-out_infinite]"}`}
                />
        ...
        {particles.map((p) => (
          <div
            ...
            style={{
              ...
              top: `${p.y}%`,
              transform:
                p.type === "rain" ? `rotate(${p.vx * 20}deg)` : "scale(1.5)",
              opacity: p.life,
            }}
          />
        ))}
        ...
        >
          <div
            className={`relative transition-all duration-300 ${isRaining ? "scale-110 translate-y-2 drop-shadow-[0_20px_20px_rgba(0,0,0,0.4)]" : "drop-shadow-lg"}`}
          >
            <Cloud
              className={`h-28 w-28 ${isRaining ? "text-slate-500 fill-slate-600 stroke-slate-700" : "text-white fill-white stroke-slate-200"}`}
              strokeWidth={1.5}
            />
            {isRaining && (
