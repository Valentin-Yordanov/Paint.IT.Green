...
import { Trophy, Leaf, Award, BookOpen, Settings, User, Plus, Target, Trash2, Languages, Moon, Loader2, Sparkles, TrendingUp, LogOut } from "lucide-react";
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-background via-secondary/30 to-primary/5">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-muted-foreground animate-pulse">{t('profile.loading') || 'Loading...'}</p>
        </div>
      </div>
    );
  }
        ...
        <Tabs defaultValue="overview" className="space-y-8">
          ...
          <TabsContent value="overview" className="space-y-8 animate-in fade-in-50 duration-500">
                ...
                <div className="px-6 pb-6 -mt-12 md:-mt-16 relative">
                  <div className="flex flex-col md:flex-row items-center gap-6">
                    ...
                    <div className="flex-1 text-center md:text-left mt-2 md:mt-8">
                      <h2 className="text-2xl md:text-3xl font-bold mb-3">{userStats.name}</h2>
                    ...
                    <div className="text-center p-4 md:p-6 rounded-2xl bg-gradient-to-br from-primary/10 to-primary/5 border border-primary/20 mt-2 md:mt-8">
                      <div className="flex items-center gap-2 justify-center">
                        ...
                        <span className="text-3xl md:text-4xl font-bold text-primary">{userStats.points.toLocaleString()}</span>
                      </div>
                      <div className="text-sm text-muted-foreground font-medium">{t('profile.points')}</div>
                    </div>
                  </div>
                </div>
              </CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
              {[
                { icon: Leaf, value: userStats.treesPlanted, label: t('profile.trees'), color: 'from-emerald-500 to-green-600' },
                { icon: Trophy, value: userStats.challengesCompleted, label: t('profile.challenges'), color: 'from-amber-500 to-orange-600' },
                { icon: BookOpen, value: userStats.lessonsFinished, label: t('profile.lessons'), color: 'from-blue-500 to-indigo-600' },
              ].map((stat, idx) => (
                <Card key={idx} className="group hover:shadow-xl transition-all duration-300 hover:-translate-y-1 border-0 shadow-lg overflow-hidden">
              ...
              <CardContent>
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-muted-foreground">{t('profile.complete')}</span>
                          </div>
                        ...
                        <Progress value={goal.progress} className="h-3 rounded-full" />
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <Dialog open={isGoalDialogOpen} onOpenChange={setIsGoalDialogOpen}>
                  ...
                  <DialogContent className="sm:max-w-[500px] rounded-2xl">
                    <DialogHeader>
                      <DialogTitle className="text-xl">{t('profile.createGoal')}</DialogTitle>
                    </DialogHeader>
                    <div className="space-y-5 pt-4">
                      <div className="space-y-2">
                        <Label className="text-sm font-medium">{t('profile.goalTitle')}</Label>
                        <Input
                          value={newGoalTitle}
                      ...
                      <div className="space-y-2">
                        <Label className="text-sm font-medium">{t('profile.goalDesc')}</Label>
                        <Textarea
          ...
          <TabsContent value="settings" className="animate-in fade-in-50 duration-500">
            <Card className="border-0 shadow-lg">
              <CardHeader className="pb-4">
                <CardTitle className="text-xl">{t('profile.settingsTitle')}</CardTitle>
              </CardHeader>
                ...
                {editMode ? (
                  <div className="space-y-5">
                    <div className="grid gap-5 md:grid-cols-2">
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="school" className="text-sm font-medium">{t('profile.school')}</Label>
                        <Input
                          id="school"
                          value={editedSchool}
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="role" className="text-sm font-medium">{t('profile.role')}</Label>
                        <Input
                          id="role"
                          value={editedRole}
                    ...
                    <div className="flex gap-3 pt-2">
                      <Button onClick={handleSaveProfile} className="h-11 px-6 rounded-xl">
                        {t('profile.save')}
                      </Button>
                      <Button variant="outline" onClick={handleCancelEdit} className="h-11 px-6 rounded-xl">
                        {t('profile.cancel')}
                      </Button>
                    </div>
                  </div>
                    ...
                    <div className="grid gap-6 md:grid-cols-2">
                      {[
                        ...
                        <div key={idx} className="p-4 rounded-xl bg-secondary/30">
                          ...
                          <p className="text-lg font-medium mt-1">{field.value}</p>
                        </div>
                      ))}
                    </div>
                    <Button onClick={() => setEditMode(true)} className="h-11 px-6 rounded-xl">
                      {t('profile.edit')}
                    </Button>
                  </div>
                )}
                  ...
                  <div className="p-5 rounded-2xl bg-gradient-to-r from-secondary/50 to-secondary/20 hover:from-secondary/70 hover:to-secondary/40 transition-all duration-300">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <Label className="text-base font-semibold">{t('profile.darkMode')}</Label>
                  ...
                  <div className="p-5 rounded-2xl bg-gradient-to-r from-secondary/50 to-secondary/20 hover:from-secondary/70 hover:to-secondary/40 transition-all duration-300">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <Label className="text-base font-semibold">{t('profile.language')}</Label>
                  ...
                  <Button 
                    ...
                    onClick={() => {
                      ...
                      toast({ 
                        ...
                        description: t('profile.loggedOutDesc') || "You have been successfully logged out." 
                      });
                    }}
                  >
